...
interface SkillSectionProps {
  setSkillInfo: (skills: InputField[]) => void; // Function to send data back to the parent component (CV)
}

const SkillSection: React.FC<SkillSectionProps> = ({ setSkillInfo }) => {
  ...
  const handleChange = (id: number, value: string) => {
    ...
    setSkillInfo(updatedInputs); // Send updated data to the parent component (CV)
  };
  ...
  const addInputs = () => {
    ...
    const newInputs = [
      ...inputs,
      { id: newId1, label: "Skill:", value: "" },
      { id: newId2, label: "Institute:", value: "" },
    ];
    setInputs(newInputs);
    setSkillInfo(newInputs); // Send updated data to the parent component (CV)
  };

  return (
    <div className="pt-16 pb-16 bg-cover bg-no-repeat" style={{ backgroundImage: "url('/images/bgnew.png')" }}>
      <section>
        <Sectionheading>Skill Section</Sectionheading>
      </section>
